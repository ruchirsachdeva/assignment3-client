...
export class AppService {
  ...
  authenticate(credentials, callback) {
    ...
    this.http.post('https://pd-social-server.herokuapp.com/api/auth', credentials, httpOptions).subscribe(response => {
  ...
  authenticateSocial(credentials, callback) {
    ...
    this.http.post('https://pd-social-server.herokuapp.com/api/auth/social', credentials, httpOptions).subscribe(response => {
      console.log('auth response.....');
      console.log(response['token']);
      localStorage.setItem('jwt', response['token']);
      return callback && callback();
